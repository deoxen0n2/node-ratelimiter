...
['redis', 'ioredis'].forEach(function (redisModuleName) {
  ...
  describe('Limiter with ' + redisModuleName, describeLimiter());
  describe('Limiter with ' + redisModuleName + ' and namespace', describeLimiter({ namespace: 'limitns' }));

  function describeLimiter(options) {
    options = options || {};
    return function() {
      ...
      describe('when multiple successive calls are made', function() {
        ...
        it('updating the count should keep all TTLs in sync', function(done) {
          ...
          limit.get(function(err, res) {
            db.multi()
              .pttl(['limit:' + (options.namespace ? options.namespace + ':' : '') + 'something:count'])
              .pttl(['limit:' + (options.namespace ? options.namespace + ':' : '') + 'something:limit'])
              .pttl(['limit:' + (options.namespace ? options.namespace + ':' : '') + 'something:reset'])
      ...
      describe('when trying to decrease before setting value', function() {
        it('should create with ttl when trying to decrease', function(done) {
          ...
          db.setex('limit:' + (options.namespace ? options.namespace + ':' : '') + 'something:count', -1, 1, function() {
            limit.get(function(err, res) {
              res.remaining.should.equal(2);
              limit.get(function(err, res) {
                res.remaining.should.equal(1);
